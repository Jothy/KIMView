...
//    this->WindowUp=8500.0;
    ...
    this->ui->widget->GetRenderWindow()->GetInteractor()->SetInteractorStyle(InteractorImage2D);
...
void ImageViewer2D::SetUpView()
{
    ...
    /*Save window and level for resetting, VTK uses it own mapping so the values are
    different to this->WindowLow & this->WindowUp */
...
void ImageViewer2D::SliceImageAndDose(double SliceLoc)
{
    ...
    AxialElements[0]=1.0;AxialElements[1]=0.0;AxialElements[2]=0.0;AxialElements[3]=0.0;
    AxialElements[4]=0.0;AxialElements[5]=1.0;AxialElements[6]=0.0;AxialElements[7]=0.0;
    AxialElements[8]=0.0;AxialElements[9]=0.0;AxialElements[10]=1.0;AxialElements[11]=0.0;
    AxialElements[12]=0.0;AxialElements[13]=0.0;AxialElements[14]=0.0;AxialElements[15]=1.0;

    double *SagittalElements=new double[16];
    SagittalElements[0]=0.0;SagittalElements[1]=0.0;SagittalElements[2]=-1.0;SagittalElements[3]=0.0;
    SagittalElements[4]=1.0;SagittalElements[5]=0.0;SagittalElements[6]=0.0;SagittalElements[7]=0.0;
    SagittalElements[8]=0.0;SagittalElements[9]=-1.0;SagittalElements[10]=0.0;SagittalElements[11]=0.0;
    SagittalElements[12]=0.0;SagittalElements[13]=0.0;SagittalElements[14]=0.0;SagittalElements[15]=1.0;

    double *CoronalElements=new double[16];
    CoronalElements[0]=1.0;CoronalElements[1]=0.0;CoronalElements[2]=0.0;CoronalElements[3]=0.0;
    CoronalElements[4]=0.0;CoronalElements[5]=0.0;CoronalElements[6]=1.0;CoronalElements[7]=0.0;
    CoronalElements[8]=0.0;CoronalElements[9]=-1.0;CoronalElements[10]=0.0;CoronalElements[11]=0.0;
    CoronalElements[12]=0.0;CoronalElements[13]=0.0;CoronalElements[14]=0.0;CoronalElements[15]=1.0;
    ...
    this->DoseLUT->Build();
    this->DoseLUT->SetTableValue(0.0,0,0,0,0);//Makes zero values totally transparent
    this->DoseLUT->Build();
...
void ImageViewer2D::on_toolButtonScreenshot_clicked()
{
    ...
    QString FileName=FileDlg->getSaveFileName(this->ui->widget,"Save File","untitled","TIFF(*.tiff)");
    //qDebug()<<FileName;
...
void ImageViewer2D::on_toolButtonContrast_clicked()
{
    ...
    {
        this->ui->widget->GetRenderWindow()->GetInteractor()->SetInteractorStyle(this->InteractorImage2D);
...
void ImageViewer2D::keyPressEvent(QKeyEvent *event)
{
    ...
    if (event->key()==Qt::Key_PageUp)
    {
        if (this->SliceOrientation==0)
        {
            this->ui->verticalScrollBar->setValue(this->ui->verticalScrollBar->value()+this->SliceStep);
        }
        else
        {
            this->ui->verticalScrollBar->setValue(this->ui->verticalScrollBar->value()-this->SliceStep);
...
void ImageViewer2D::on_actionMeasureDistance_triggered()
{
    if (this->ui->actionMeasureDistance->isChecked())
    {
        ...
        vtkAxisActor2D *axis =  static_cast<vtkDistanceRepresentation2D *>(this->DistanceWidget->GetRepresentation())->GetAxis();
...
void ImageViewer2D::on_actionMeasureAngle_triggered()
{
    if (this->ui->actionMeasureAngle->isChecked())
    {
        ...
        this->AngleWidget->SetInteractor(this->ui->widget->GetRenderWindow()->GetInteractor());
        ...
        static_cast<vtkAngleRepresentation2D *>(this->AngleWidget->GetRepresentation())->
                GetRay1()->GetProperty()->SetColor(1,0,0);
        static_cast<vtkAngleRepresentation2D *>(this->AngleWidget->GetRepresentation())->
                GetRay2()->GetProperty()->SetColor(0,1,0);
        static_cast<vtkAngleRepresentation2D *>(this->AngleWidget->GetRepresentation())->
                GetArc()->GetProperty()->SetColor(0,0,1);
...
vtkSmartPointer<vtkActor>ImageViewer2D::CutROI(vtkPolyData *mesh,double sliceNo,double R,double G,double B,int Orientation)
{
...
void ImageViewer2D::DisplayPOIs(double SliceLoc, int Orientation)
{
    ...
    if(this->POIVisibility==1)
    {
        ...
        for(int i=0;i<this->POIList.size();i++)
        {
            ...
            vtkSmartPointer<vtkActor>POIActor=
                    vtkSmartPointer<vtkActor>::New();
...
void ImageViewer2D::on_toolButtonProbe_clicked()
{
    if(this->RTDose->GetScalarRange()[1]>1.0)
    {
        ...
        if(this->ui->toolButtonProbe->isChecked())
        {
            ...
            this->lineWidget->GetLineRepresentation()->GetEndPointProperty()->SetColor(0,255,0);//starting point green
            this->lineWidget->GetLineRepresentation()->GetEndPoint2Property()->SetColor(255,0,0);//starting point red

            static_cast<vtkLineRepresentation *>(this->lineWidget->GetRepresentation())->SetLineColor(1,1,1);
            static_cast<vtkLineRepresentation *>(this->lineWidget->GetRepresentation())->GetLineProperty()->SetLineWidth(0.25);
            static_cast<vtkLineRepresentation *>(this->lineWidget->GetRepresentation())->GetLineProperty()->SetAmbient(0.0);
            static_cast<vtkLineRepresentation *>(this->lineWidget->GetRepresentation())->SetResolution(1000);
            static_cast<vtkLineRepresentation *>(this->lineWidget->GetRepresentation())->GetLineProperty()->SetOpacity(1.0);

            //z=-5 so the widget always stays on top of the slice
            double lineBds[6]={-50,50,0,0,-5,-5};//initial length to 10cm
            static_cast<vtkLineRepresentation *>(this->lineWidget->GetRepresentation())->PlaceWidget(lineBds);
